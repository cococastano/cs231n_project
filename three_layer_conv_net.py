...
loader_train = torch.utils.data.DataLoader(train, shuffle=False)
val = torch.utils.data.TensorDataset(X_val, y_val)
loader_val = torch.utils.data.DataLoader(val, shuffle=False)
test = torch.utils.data.TensorDataset(X_test, y_test)
loader_test = torch.utils.data.DataLoader(test, shuffle=False)
...
def random_weight(shape):
    ...
    # a fully connected weight has shape 2
    if len(shape) == 2:
        fan_in = shape[0]
    # conv weight is [out_channel, in_channel, kH, kW]
    else:
...
loss_history, val_acc_history, train_acc_history = \
    train_model(model, optimizer, epochs=2, return_history=True)
